...
MIN_OBJECT_HEIGHT_IN_PIXELS = -np.inf  # irrelevant for TrackRCNN detections?
...
def read_detections(
    ...
    ----------------------------------------------------------------------------
       ...
       1    score        Float, indicating confidence in detection, higher is
                         better.
       1    class        Detection class, 1 being for cars and 2 for pedestrians

       1    image height
       1    image width

       1    mask         RLE encoded segmentation mask 'counts'

       128  reid_vector  Reassociation vector (TrackRCNN)
    ...
    for line in lines:
        words = line.strip().split(' ')
        assert len(words) == 138, "Invalid number of elements in line."
        object_class_numeric = int(words[6])
        if object_class_numeric == 1:
            object_class = 'Car'
        elif object_class_numeric == 2:
            object_class = 'Pedestrian'
        else:
            warnings.warn('Invalid object class, skipping detection.')
            object_class = ''

        if object_class not in object_classes:
            continue
        frame_idx = int(words[0])
        roi = np.asarray([float(x) for x in words[1:5]])
        roi[2:] -= roi[:2] - 1  # Convert to x, y, w, h
        if roi[3] < min_height:
            continue
        ...
        rle_mask = dict()
        rle_mask['size'] = [words[7], words[8]]
        rle_mask['counts'] = words[9]

        reid_vector = np.array([float(x) for x in words[10:10+128]])

        detections.setdefault(frame_idx, []).append(
            pymotutils.MaskBasedDetection(frame_idx, roi, confidence, feature=reid_vector, rle_mask=rle_mask))
...
def num2trackid(num, padding):
    num_str = str(num)
    leading_zeros_num = padding - len(num_str)
    return '0' * leading_zeros_num + num_str


def write_hypotheses_mots(filename, track_set, object_class):
    lines = []
    for frame_idx in track_set.frame_range():
        curr_frame_detections = track_set.collect_detections(frame_idx)
        for track_id, detection in curr_frame_detections.items():
            if object_class == 'Car':
                cat_id = 1
            ...
            new_track_id = str(cat_id) + num2trackid(track_id, 3)

            line = (
                str(frame_idx) + ' ' + str(new_track_id) + ' ' + str(cat_id) + ' ' + str(detection.rle_mask['size'][0]) + ' ' + str(
                    detection.rle_mask['size'][1]) + ' ' + detection.rle_mask['counts'] + os.linesep)
...
class DataSource(pymotutils.DataSource):
    ...
    def read_frame_data(self, frame_idx):
        ...
        if self.sensor_poses is not None:
...
class Devkit(object):
    ...
    def create_data_source(
        ...
        if os.path.exists(oxts_filename):
            imu_to_world_list = read_odometry(oxts_filename)
        else:
            imu_to_world_list = None
        ...
        if os.path.exists(calibration_filename):
            ...
            imu_to_rectified = None

        if imu_to_rectified is not None and imu_to_world_list is not None:
